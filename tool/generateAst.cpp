...
void defineAst(std::string                        outputDir,
    ...
    {
        std::cerr << "Could not open file for writing: " << outputDir + "/" + baseName + ".hpp"
                  << '\n';
        exit(1);
    }
    ...
    for (const std::string& type : types)
    {
        ...
        subClasses.push_back(type.substr(0, type.find(":"))
                               .substr(type.find_first_not_of(""), type.find_last_not_of(" ") + 1));
    }
    ...
    for (const std::string& subclassName : subClasses)
    {
        ...
        headerFile << "virtual std::string visit" << subclassName << baseName << "(const "
                   << subclassName << " &" << baseName << ") = 0;"
                   << "\n";
    }
    headerFile << "};\n\n";
    ...
    headerFile << "virtual ~" << baseName << "() = default;"
               << "\n";
    headerFile << "virtual std::string accept(" << baseName << "Visitor &visitor) = 0;"
               << "\n";
    headerFile << "};"
               << "\n";
...
void defineType(std::ofstream& headerFile,
                std::string&   baseName,
    ...
    while (std::getline(iss, field, ','))  // Takes in params by reference
    {
        ...
        fields.push_back(
          std::make_pair(field.substr(0, field.find(" ")), field.substr(field.find(" ") + 1)));
    }
    ...
    headerFile << "std::string accept( " << baseName
               << "Visitor &visitor) override { return visitor.visit" << className << baseName
               << "(*this); }"
               << "\n";
